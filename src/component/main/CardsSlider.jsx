            ...
            >
                {/* {
                    data ?  data[category]
                        ...
                        .map((item, i) => (
                            <SwiperSlide key={i}>
                                <Link to={`${i + 1}`} className="text-center w-[180px] border rounded-md p-3 bg-white relative">
                                    <GoHeart onClick={() => { setWishList() }} className='absolute cursor-pointer top-3 right-3 text-[1.3em] text-[#FF8300]' />
                                    <img src={item.img} alt="" />
                                    <h5 className="py-4 hover:text-[#FF8300] h-16 text-[.7em] font-semibold">{item.title}</h5>
                                    <p className="font-bold text-[1.3em]">{item.price} ₼</p>
                                    <div className="py-3">
